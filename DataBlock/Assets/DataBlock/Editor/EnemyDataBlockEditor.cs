...
[CustomEditor(typeof(EnemyDataBlock))]
public sealed class EnemyDataBlockEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("エネミーデータテキスト再構築")) Run(Operation.Rebuild);
    ...
    private void Run(Operation operation)
    {
        try
        {
            var block = (EnemyDataBlock)target;
            switch (operation)
            {
                case Operation.Normalize:
                    _text = EnemyTextConverter.Normalize(_text);
                    _message = "構文を統一しました。シリアライズデータは変更していません。";
                    break;
                case Operation.Deserialize:
                    EnemyTextData parsed = EnemyTextConverter.Parse(_text);
                    Undo.RecordObject(block, "Set enemy data");
                    block.Data = parsed;
                    if (PrefabUtility.IsPartOfPrefabInstance(block)) PrefabUtility.RecordPrefabInstancePropertyModifications(block);
                    EditorUtility.SetDirty(block);
                    serializedObject.Update();
                    _message = "エネミー・行動・ドロップをセットしました。入力テキストは変更していません。";
                    break;
                case Operation.Rebuild:
                    _text = EnemyTextConverter.Build(block.Data);
                    _message = "シリアライズデータからエネミーテキストを再構築しました。";
                    break;
                case Operation.Json:
